...
document.addEventListener('DOMContentLoaded',()=>{
  const btn=document.getElementById('saveBtn'); if(btn) btn.addEventListener('click',saveMood);
  const clr=document.getElementById('clearAllBtn'); if(clr) clr.addEventListener('click',clearAll);
  const rg=document.getElementById('rangeSelect'); if(rg) rg.addEventListener('change',render);
  const exp=document.getElementById('exportBtn'); if(exp) exp.addEventListener('click',exportCSV);
  render();
});
function saveMood(){const moodEl=document.getElementById('mood'); const noteEl=document.getElementById('note'); const mood=moodEl?moodEl.value:2; const note=noteEl?noteEl.value:''; const ts=nowTimestamp(); records.push({timestamp:ts,mood:parseInt(mood),note}); persist(); if(noteEl) noteEl.value=''; render();}
